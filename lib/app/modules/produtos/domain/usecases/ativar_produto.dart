import 'package:dartz/dartz.dart';
import 'package:pdvexpress/app/core/domain/erros/error_order.dart';
import 'package:pdvexpress/app/core/domain/repositories/produtos_repository.dart';

abstract class AtivarProduto {
  Future<Either<ErrorOrder, bool>> call(String uid);
}

class AtivarProdutoImpl implements AtivarProduto {
  final ProdutoRepository repository;

  AtivarProdutoImpl(this.repository);

  @override
  Future<Either<ErrorOrder, bool>> call(String uid) async => repository.activated(uid);
}
